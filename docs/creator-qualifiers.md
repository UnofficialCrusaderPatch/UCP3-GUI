...
A group shows Mixed when only some editable options are locally Required, including when its other options are untouched. The group action still operates only on local overrides; if all those overrides are Required, clicking suggests them, even while untouched siblings make the parent display Mixed.
