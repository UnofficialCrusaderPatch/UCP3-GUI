...
const UIFactory = {
  ...
  CreateUCP2RadioGroup(args: {
    ...
    if (hasHeader) {
      headerElement = (
        <div className="sword-checkbox ucp2-radio-group">
          <input
            type="checkbox"
            className="me-2"
            id={`${url}-header`}
          ...
          />
          <label className="" htmlFor={`${url}-header`} />
          <span className="ms-4">{header}</span>
        </div>
      );
    }
    return (
      <div className="col-auto" style={{ marginLeft: 0, marginBottom: 0 }}>
        {headerElement}
        <div>
          <div className="form-check-label" htmlFor={`${url}-choice`}>
            {!hasHeader && header}
            {text}
          </div>
        </div>
        <div className="row">
          ...
          >
            {choices.map((choice) => (
              ...
              <div key={choice.name} className="sword-checkbox ms-2">
                <input
                  type="radio"
                  name={url}
                  className="me-2"
                  ...
                  onChange={(event) => {
                    setConfiguration({
                      type: 'set-multiple',
                      value: Object.fromEntries([
                        [url, { ...value, ...{ choice: event.target.value } }],
                      ]),
                    });
                    setConfigurationTouched({
                      type: 'set-multiple',
                    ...
                    configuration[url] = event.target.value;
                  }}
                />
                <label
  ...
  CreateUCP2Slider(args: {
    ...
    if (hasHeader) {
      headerElement = (
        <div className="sword-checkbox ucp2-slider">
          <input
            type="checkbox"
            className="me-2"
            id={`${url}-header`}
            ...
            disabled={!isEnabled || disabled}
          />
          <label className="switch-label" htmlFor={`${url}-header`}/>
          <span className="ms-4">{header}</span>
        </div>
      );
    }
    ...
    return (
      <div className="" style={{ marginLeft: 0, marginBottom: 0 }}>
        {headerElement}
        <div>
          <div className="form-check-label" htmlFor={`${url}-slider`}>
            {!hasHeader && header}
            {text}
          </div>
        </div>
    ...
    const headerElement = (
      <div className="sword-checkbox ucp2-switch">
        <input
          ...
          id={`${url}`}
          key={`${url}-switch`}
          checked={value === undefined ? false : (value as boolean)} // TODO check if enabled is needed
          onChange={(event) => {
            setConfiguration({
              type: 'set-multiple',
          ...
          disabled={!isEnabled || disabled}
        />
        <label className="switch-label" htmlFor={`${url}`} />
        <span className="ms-4">{header}</span>
      </div>
    );
  ...
  CreateUCP2SliderChoice(args: {
    ...
    if (hasHeader) {
      headerElement = (
        <div className="sword-checkbox ucp2-slider-choice">
          <input
            type="checkbox"
            className="me-2"
            id={`${url}-header`}
    ...
    const radios = choices.map(
      (choice: {
        ...
        return (
          // eslint-disable-next-line jsx-a11y/label-has-associated-control
          <div key={choice.name} className="">
            <div className="sword-checkbox ms-2">
              <input
                type="radio"
                name={url}
                value={choice.name}
            ...
            <div className="col ms-2" style={{ marginLeft: 2, marginBottom: 0 }}>
              <div className="row">
                <div className="col-auto">
                  <Form.Label
                    disabled={
  ...
  CreateSwitch(args: {
    ...
    return (
      <div className="d-flex align-items-baseline lh-sm {className}">
        {hasWarning ? (
  ...
  CreateNumberInput(args: {
    ...
    return (
      <Form.Group
        className={`d-flex align-items-baseline lh-sm config-number-group ${className}`}
      >
        {hasWarning ? (
          <UIFactory.ConfigWarning
            text={configurationWarnings[url].text}
  ...
  CreateSection(args: {
    ...
    const elements = (contents.elements as DisplayConfigElement[]).map(
      (el: DisplayConfigElement) => {
        ...
        return (
          <div className="transparent-white-bg">
            <UIFactory.CreateUIElement
              key={key}
    ...
    // level headings for settings (e.g. h3)
    const htmlHeader = React.createElement(
      `h${level + 1}`,
      { className: `my-0 ${level < 2 ? 'text-center fw-bold' : ''}` },
      header
    );
    ...
    return (
      ...
      <div
        key={identifier}
        id={identifier}
        style={{ marginLeft: `0rem`, paddingTop: `${(3 - level) * 0.1}rem` }}
      >
        {htmlHeader}
      ...
      content: (
        ...
        >
          <div id="config-General">
            <h2 id="config-General" className="mb-0 text-center fw-bold">{t('gui-editor:config.general')}</h2>
            {elements}
          </div>
          <div>{children}</div>
        </div>
