...
export function LanguageSelect({ closeLangSelect }: { closeLangSelect: () => void }) {
        ...
        <div className="container h-50 border border-info rounded p-4 p-4">
            <div className="row justify-content-center h-100">
                <div className="col-8 d-flex flex-column h-100">
                    <div className="">
                        <label htmlFor="language-select">{t("gui-landing:select.language")}</label>
                        <input
                            id="language-select"
                            type="text"
                            ...
                            value={(languages as { [key: string]: string })[configHandler.getLanguage() as string]}
                        />
                    </div>
                    <div
                        id="language-select"
                        className={`list-group overflow-auto bg-light${selectActive ? '' : ' invisible'}`}
                        style={{minHeight: "0"}}
                        onClick={(event) => {
