...
import {
  ...
  readLocales,
  readConfig,
} from '../../../../function/extensions/discovery/io';
...
import {
  MINISEARCH_ATOM,
  SEARCH_QUERY_ATOM,
} from '../ui-elements/ui-factory/sections/search';
...
vi.mock('../../../general/message', () => ({
  useMessage: () => (message: string | { key: string }) =>
    typeof message === 'string' ? message : message.key,

  default: ({ message }: { message?: string | { key: string } }) =>
    !message ? null : typeof message === 'string' ? message : message.key,
}));

vi.mock('../../../../function/game-folder/utils', () => ({
  useCurrentGameFolder: () => 'C:/fixture',
  getCurrentGameFolder: () => 'C:/fixture',
}));

vi.mock('../../../../tauri/tauri-dialog', () => ({
...
async function fixture(folder: string) {
  ...
  const handle = {
    ...
    getTextContents: (file: string) =>
      readFile(resolvePath(path, file), 'utf8'),

    doesEntryExist: async (file: string) =>
      file === 'locale/' ||
      readFile(resolvePath(path, file))
        .then(() => true)
        .catch(() => false),
  } as ExtensionHandle;
  ...
  const ext = {
    ...
    io: {
      path,
      handle: async (fn: (h: ExtensionHandle) => unknown) => fn(handle),
    },
  } as Extension;
...
function install(ext: Extension) {
  store.set(EXTENSION_STATE_INTERNAL_ATOM, {
    ...
    explicitlyActivatedExtensions: [ext],
    configuration: {
      ...createEmptyConfigurationState(),
      defined: getConfigDefaults(ext.ui as unknown as OptionEntry[]),
    },
  });

  store.set(CONFIGURATION_FULL_REDUCER_ATOM, {
    type: 'reset',
    value: getConfigDefaults(ext.ui as unknown as OptionEntry[]),
  });
...
function snapshot(ext: Extension) {
  return toYaml(
    serializeUCPConfig(
      store.get(CONFIGURATION_USER_REDUCER_ATOM),
      store.get(CONFIGURATION_FULL_REDUCER_ATOM),
      [ext],
      [ext],
      store.get(CONFIGURATION_QUALIFIER_REDUCER_ATOM),
    ),
  );
}
...
describe('declarative options.yml modals', () => {
  test('discovery attaches every descendant and localizes real resource table fields', async () => {
    ...
    const visit = (node: DisplayConfigElement) => {
      expect(node.extension.name).toBe('modal-resources');
      displayChildren(node).forEach(visit);
    };

    (ext.ui as unknown as DisplayConfigElement[]).forEach(visit);

    const german = applyLocale(
      ext,
      ext.locales.de,
    ) as unknown as DisplayConfigElement[];

    expect(german[0]).toHaveProperty(
      'header',
      'Startressourcen und Vorr\u00e4te',
    );

    const table = displayChildren(german[0])[0];

    expect(displayChildren(displayChildren(table)[0])[0]).toHaveProperty(
      'text',
      'Holz',
    );

    expect(
      getConfigDefaults(ext.ui as unknown as OptionEntry[])[
  ...
  test('rejects malformed children and accepts an intentionally empty menu', async () => {
    const handle = {
      doesEntryExist: async () => true,
      getTextContents: async () =>
        'options: [{name: bad, display: Modal, children: false}]',
  ...
  test('opening, filtering, dismissing and reopening are clean; filters are independent', async () => {
    ...
    const opener = screen.getByRole('button', { name: 'Mixed controls' });
    opener.focus();
  ...
  test('same discovered GroupBox and Modal children produce identical working and serialized configuration', async () => {
    ...
    const group = {
      ...ext,
      ui: ext.ui.map((node) => ({ ...node, display: 'GroupBox' })),
    };
    ...
    expect(
      ((await screen.findByLabelText('Amount')) as HTMLInputElement).value,
    ).toBe('42');
  ...
  test('propagates inherited disabled state, reacts while open, and keeps required locks', async () => {
    ...
    act(() => {
      const state = store.get(EXTENSION_STATE_INTERNAL_ATOM);
      store.set(EXTENSION_STATE_INTERNAL_ATOM, {
        configuration: {
          ...state.configuration,
          locks: {
            'modal-mixed.locked': { lockedBy: 'preset', lockedValue: 25 },
          },
        },
      });
    });
    ...
    expect(
      (screen.getByLabelText('Required amount') as HTMLInputElement).disabled,
    ).toBe(true);
  ...
  test('nested dialogs return to their parent; owner removal cleans the complete stack', async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByRole('dialog', { name: 'Mixed controls' }),
      ).toBeTruthy(),
    );
...
test('Creator qualifiers, required/suggested presets, reset and YAML plugin round-trip use shared state', async () => {
  ...
  install(ext);
  const state = buildExtensionConfigurationDB({
    ...store.get(EXTENSION_STATE_INTERNAL_ATOM),
    activeExtensions: [preset, ext],
  });
  store.set(EXTENSION_STATE_INTERNAL_ATOM, state);
  store.set(CONFIGURATION_FULL_REDUCER_ATOM, {
    type: 'reset',
    value: state.configuration.defined,
  });
  ...
  const amount = await screen.findByLabelText('Amount');
  expect(
    (screen.getByLabelText('Required amount') as HTMLInputElement).value,
  ).toBe('25');
  expect(
    (screen.getByLabelText('Required amount') as HTMLInputElement).disabled,
  ).toBe(true);
  expect(
    state.configuration.suggestions['modal-mixed.suggested'].suggestedValue,
  ).toBe(30);
  ...
  expect(
    parsed.configEntries['modal-mixed.amount'].contents['required-value'],
  ).toBe(43);
  const plugin = { ...preset, configEntries: parsed.configEntries };
  const restored = buildExtensionConfigurationDB({
    ...state,
    activeExtensions: [plugin, ext],
  });
  expect(restored.configuration.defined['modal-mixed.amount']).toBe(43);
  expect(restored.configuration.locks['modal-mixed.amount'].lockedValue).toBe(
    43,
  );
  fireEvent.click(
  ...
  expect(
    store.get(CONFIGURATION_USER_REDUCER_ATOM)['modal-mixed.amount'],
  ).toBeUndefined();
});
...
test('removing the active extension dismisses its menu and outstanding file results cannot write', async () => {
  ...
  vi.mocked(openFileDialog).mockImplementationOnce(
    () =>
      new Promise((resolve) => {
        finish = resolve;
      }),
  );
  const fileButtons = within(
    screen.getByRole('dialog', { name: 'Files and speed' }),
  ).getAllByRole('button');
  const picker = fileButtons.find((button) =>
  ...
  expect(screen.queryByRole('dialog')).toBeNull();
  await act(async () => {
    finish(Option.of('C:/fixture/test.txt'));
  });
  expect(
    store.get(CONFIGURATION_USER_REDUCER_ATOM)['modal-mixed.file'],
  ).toBeUndefined();
});
...
test('legacy switches, radio groups and sliders render through the unchanged factory', async () => {
  ...
  fireEvent.click(screen.getByRole('button', { name: 'Mixed controls' }));
  fireEvent.click(
    await screen.findByRole('button', { name: 'Legacy controls' }),
  );
  const dialog = await screen.findByRole('dialog', { name: 'Legacy controls' });
  expect(
    within(dialog).getAllByRole('checkbox', { hidden: true }).length,
  ).toBeGreaterThanOrEqual(3);
  expect(
    within(dialog).getAllByRole('slider', { hidden: true }).length,
  ).toBeGreaterThanOrEqual(2);
  fireEvent.click(screen.getByRole('button', { name: 'close' }));
  expect(screen.getByRole('dialog', { name: 'Mixed controls' })).toBeTruthy();
});

test('StrictMode effect replay keeps an opened modal usable and scoped cleanup restores focus', async () => {
  const ext = await fixture('modal-mixed-1.0.0');
  install(ext);
  render(
    <StrictMode>
...
test('searching a modal or group heading reveals its controls instead of an empty result', async () => {
  const ext = await fixture('modal-mixed-1.0.0');
  install(ext);
  store.set(SEARCH_QUERY_ATOM, 'Mixed controls');
  render(
    <>
      <CreateSections />
      <Overlay />
  ...
  fireEvent.change(screen.getByRole('searchbox'), {
    target: { value: 'Advanced settings' },
  });
  expect(
    screen.getByRole('spinbutton', { name: 'Suggested amount' }),
  ).toBeTruthy();
});
