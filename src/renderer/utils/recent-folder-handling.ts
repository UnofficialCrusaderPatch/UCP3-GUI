...
export class RecentFolderHandler {
    ...
    async #getRecentFoldersFilePath(): Promise<string> {
        ...
        this.#recentFoldersFilePath = `${await getBaseFolder()}${RecentFolderHandler.#recentFoldersFileName}`;
        return this.#recentFoldersFilePath;
    }
    ...
    #sortRecentFolders(): void {
        if (this.#currentRecentFolders) {
            this.#currentRecentFolders.sort((recentFolderOne, recentFolderTwo) => recentFolderTwo.date - recentFolderOne.date);
        }
    }
    ...
    async saveRecentFolders() {
        try {
            await writeTextFile(await this.#getRecentFoldersFilePath(), JSON.stringify(this.#getCurrentRecentFolders()));
        } catch (error) {
    ...
    addToRecentFolders(path: string) {
        ...
        const alreadyThereIndex = this.#currentRecentFolders.findIndex(recentFolder => recentFolder.path === path);
        if (alreadyThereIndex < 0) {
            this.#currentRecentFolders.push({ path: path, date: Date.now() });
        } else {
